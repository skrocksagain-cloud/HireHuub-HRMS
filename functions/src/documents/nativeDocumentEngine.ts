...
import { compileOfferLetterHTML, type OfferBlock } from './renderers/nativeOfferRenderer';
import { compileRelievingLetterHTML } from './renderers/nativeRelievingRenderer';
import { compileIncrementLetterHTML } from './renderers/nativeIncrementRenderer';
import { compilePayslipHTML } from './renderers/nativePayslipRenderer';
import { compileInvoiceHTML, type HireHuubTemplateType } from './renderers/nativeInvoiceRenderer';
import { getOfferLetterHTML } from './templates/offerLetterTemplate';



export interface NativeDocumentGenerationOptions {
  documentType: string;
  brandId: string;
  entityId: string;
...
export interface NativeDocumentGenerationResult {
  success: boolean;
  documentId: string;
  documentType: string;
  fileName: string;
 ...
 */
export class NativeDocumentEngine {
  static async generateOfferLetter(options: NativeDocumentGenerationOptions): Promise<NativeDocumentGenerationResult> {
    ...
    const templateRef = adminDb.collection('document_templates').doc(templateDocId);
    const templateSnap = await templateRef.get();

    let blocksToRender: OfferBlock[] | null = null;
    let versionNum = 1;
    ...
    if (templateSnap.exists) {
      ...
      if (versionSnap.exists) {
        ...
        blocksToRender = (verConfig.offerSchema?.blocks || verData.offerSchema?.blocks) as OfferBlock[];
      } else if (mainData.offerSchema?.blocks) {
        blocksToRender = mainData.offerSchema.blocks as OfferBlock[];
      }
    }
    ...
    const specificSignatures: Record<string, { signatureUrl: string; fullName: string; designation: string }> = {};

    try {
      const companySnap = await adminDb.collection('admin_company').doc('hirehuub_company_settings').get();
      if (companySnap.exists) {
        const compData = companySnap.data() || {};
        const brandList = (compData.brandProfilesList || []) as BrandProfileRecord[];
        ...
        const signatories = (compData.signatoriesV2 || compData.signatures || []) as SignatoryRecord[];
        ...
        signatories.forEach((s, idx) => {
          ...
          if (s.id) specificSignatures[s.id] = sigData;
          if (s.signatoryId) specificSignatures[s.signatoryId] = sigData;
        });
      }
    ...
    let htmlContent = '';
    try {
      htmlContent = compileOfferLetterHTML({
      ...
      htmlContent = getOfferLetterHTML(placeholders);
    }
    ...
    await adminDb.collection('documents').doc(docRecordId).set(documentRecord);
  ...
  static async generateRelievingLetter(options: NativeDocumentGenerationOptions): Promise<NativeDocumentGenerationResult> {
    const { documentType, brandId, entityId, placeholders, generatedBy = 'Super Admin' } = options;
    ...
    try {
      ...
      if (companySnap.exists) {
        ...
        if (matchedBrand) {
          brandLogoUrl = matchedBrand.logoUrl || compData.logoUrl || '';
          brandStampUrl = matchedBrand.stampUrl || compData.stampUrl || '';
          if (!placeholders.BRAND_NAME) placeholders.BRAND_NAME = matchedBrand.brandName || '';
        }

        if (!placeholders.LEGAL_NAME) placeholders.LEGAL_NAME = compData.legalName || compData.companyName || matchedBrand?.brandName || '';
        if (!placeholders.COMPANY_NAME) placeholders.COMPANY_NAME = placeholders.LEGAL_NAME;
        if (!placeholders.BRAND_ADDRESS) placeholders.BRAND_ADDRESS = (matchedBrand as any)?.address || compData.address || '';
        if (!placeholders.ADDRESS) placeholders.ADDRESS = placeholders.BRAND_ADDRESS;
        if (!placeholders.BRAND_EMAIL) placeholders.BRAND_EMAIL = (matchedBrand as any)?.email || compData.email || '';
        if (!placeholders.EMAIL) placeholders.EMAIL = placeholders.BRAND_EMAIL;
        if (!placeholders.BRAND_PHONE) placeholders.BRAND_PHONE = (matchedBrand as any)?.phone || compData.phone || '';
        if (!placeholders.PHONE) placeholders.PHONE = placeholders.BRAND_PHONE;
        if (!placeholders.BRAND_WEBSITE) placeholders.BRAND_WEBSITE = (matchedBrand as any)?.website || compData.website || '';
        if (!placeholders.WEBSITE) placeholders.WEBSITE = placeholders.BRAND_WEBSITE;
        if (!placeholders.CIN) placeholders.CIN = compData.cin || '';
        if (!placeholders.PAN) placeholders.PAN = compData.pan || '';
        if (!placeholders.GSTIN) placeholders.GSTIN = compData.gstin || '';
    ...
    // 4. Render Published Schema Blocks to HTML
    const htmlContent = compileRelievingLetterHTML({
      blocks: blocksToRender,
      placeholders,
      brandLogoUrl,
      brandStampUrl,
  ...
  static async generateIncrementLetter(
    options: NativeDocumentGenerationOptions
  ): Promise<NativeDocumentGenerationResult> {
    ...
    let blocksToRender: OfferBlock[] = [];
    let versionNum = 1;

    try {
      const tmplSnap = await adminDb.collection('document_templates').doc(templateDocId).get();
      ...
      if (tmplSnap.exists) {
        ...
        if (tmplData) {
          ...
          if (blocks.length > 0) {
            blocksToRender = blocks as OfferBlock[];
    ...
    try {
      ...
      if (companySnap.exists) {
        ...
        const matchedBrand = brandProfiles.find((b: any) => b.id === brandId || b.brandId === brandId);
        ...
        const activeSignatories = signatories.filter((s: any) => s.isActive !== false);
        const primarySig = activeSignatories.find((s: any) => s.isPrimary) || activeSignatories[0] || signatories.find((s: any) => s.isPrimary) || signatories[0];
        if (primarySig) {
        ...
        signatories.forEach((s: any, idx: number) => {
          const sigKey = s.id || s.signatoryId || `sig-${idx}`;
          const sigData = {
            signatureUrl: s.signatureUrl || s.url || '',
            fullName: s.fullName || s.name || '',
        ...
        if (!placeholders.LEGAL_NAME) placeholders.LEGAL_NAME = legalName;
        if (!placeholders.COMPANY_NAME) placeholders.COMPANY_NAME = legalName;
        if (!placeholders.BRAND_NAME) placeholders.BRAND_NAME = brandName;
        if (!placeholders.BRAND_ADDRESS) placeholders.BRAND_ADDRESS = brandAddress;
        if (!placeholders.ADDRESS) placeholders.ADDRESS = brandAddress;
        if (!placeholders.BRAND_EMAIL) placeholders.BRAND_EMAIL = brandEmail;
        if (!placeholders.EMAIL) placeholders.EMAIL = brandEmail;
        if (!placeholders.BRAND_PHONE) placeholders.BRAND_PHONE = brandPhone;
        if (!placeholders.PHONE) placeholders.PHONE = brandPhone;
        if (!placeholders.BRAND_WEBSITE) placeholders.BRAND_WEBSITE = brandWebsite;
        if (!placeholders.WEBSITE) placeholders.WEBSITE = brandWebsite;
        if (!placeholders.CIN) placeholders.CIN = cin;
        if (!placeholders.PAN) placeholders.PAN = pan;
        if (!placeholders.GSTIN) placeholders.GSTIN = gstin;
      }
    } catch {
      // Fallback
    }

    const htmlContent = compileIncrementLetterHTML({
      blocks: blocksToRender,
      placeholders,
      brandLogoUrl,
      brandStampUrl,
  ...
  static async generatePayslip(
    ...
    try {
      ...
      if (companySnap.exists) {
        ...
        const signatories = cData.signatoriesV2 || [];
        const activeSignatories = signatories.filter((s: any) => s.isActive !== false);
        const primarySig = activeSignatories.find((s: any) => s.isPrimary) || activeSignatories[0];
        if (primarySig) {
          defaultSignatureUrl = primarySig.signatureUrl || primarySig.url || '';
          defaultSignatoryName = primarySig.fullName || primarySig.name || '';
          defaultSignatoryDesignation = primarySig.designation || '';
        ...
        if (!placeholders.COMPANY_NAME) placeholders.COMPANY_NAME = legalName;
        if (!placeholders.BRAND_NAME) placeholders.BRAND_NAME = brandName;
        if (!placeholders.BRAND_ADDRESS) placeholders.BRAND_ADDRESS = matchedBrand?.address || cData.address || '';
        if (!placeholders.BRAND_PHONE) placeholders.BRAND_PHONE = matchedBrand?.phone || cData.phone || '';
        if (!placeholders.BRAND_EMAIL) placeholders.BRAND_EMAIL = matchedBrand?.email || cData.email || '';
        if (!placeholders.BRAND_WEBSITE) placeholders.BRAND_WEBSITE = matchedBrand?.website || cData.website || '';
      }
    } catch {
      // Fallback
    }

    const htmlContent = compilePayslipHTML({
      blocks: [],
      placeholders,
      brandLogoUrl,
      defaultSignatureUrl,
    ...
    if (entityId) {
      await adminDb.collection('generated_payslips').doc(entityId).set(
        {
          storagePath,
          documentId: docRecordId,
  ...
  static async generateInvoice(
    options: NativeDocumentGenerationOptions & { templateType?: HireHuubTemplateType }
  ): Promise<NativeDocumentGenerationResult> {
    ...
    try {
      ...
      if (companySnap.exists) {
        ...
        if (!placeholders.COMPANY_STAMP_URL) placeholders.COMPANY_STAMP_URL = cData.stampUrl || '';

        const signatories = cData.signatoriesV2 || [];
        const activeSignatories = signatories.filter((s: any) => s.isActive !== false);
        let matchedSig = null;
        if (placeholders.SIGNATORY_ID) {
          matchedSig = activeSignatories.find((s: any) => s.id === placeholders.SIGNATORY_ID);
        }
        if (!matchedSig && placeholders.SIGNATORY_NAME) {
          matchedSig = activeSignatories.find((s: any) => (s.fullName || s.name || '').trim().toLowerCase() === placeholders.SIGNATORY_NAME.trim().toLowerCase());
        }
        if (!matchedSig) {
          matchedSig = activeSignatories.find((s: any) => s.isPrimary) || activeSignatories[0];
        }

        if (matchedSig) {
          defaultSignatoryName = matchedSig.fullName || matchedSig.name || '';
          if (!placeholders.SIGNATORY_NAME) placeholders.SIGNATORY_NAME = defaultSignatoryName;
          if (!placeholders.SIGNATORY_DESIGNATION) placeholders.SIGNATORY_DESIGNATION = matchedSig.designation || '';
          if (!placeholders.SIGNATURE_URL) placeholders.SIGNATURE_URL = matchedSig.signatureUrl || '';
        }

        const legalName = cData.companyName || cData.legalName || '';
        if (!placeholders.LEGAL_NAME) placeholders.LEGAL_NAME = legalName;
        if (!placeholders.COMPANY_NAME) placeholders.COMPANY_NAME = legalName;
        if (!placeholders.BRAND_NAME) placeholders.BRAND_NAME = cData.brandName || 'Hire Huub';
        if (!placeholders.BRAND_ADDRESS) placeholders.BRAND_ADDRESS = cData.address || '';
        if (!placeholders.BRAND_PHONE) placeholders.BRAND_PHONE = cData.phone || '';
        if (!placeholders.BRAND_EMAIL) placeholders.BRAND_EMAIL = cData.email || '';
        if (!placeholders.BRAND_WEBSITE) placeholders.BRAND_WEBSITE = cData.website || '';
        if (!placeholders.COMPANY_GSTIN) placeholders.COMPANY_GSTIN = cData.gstin || '';
        if (!placeholders.COMPANY_PAN) placeholders.COMPANY_PAN = cData.pan || '';
        if (!placeholders.REGISTERED_STATE) placeholders.REGISTERED_STATE = cData.registeredState || 'West Bengal';

        if (cData.bankDetails) {
          if (!placeholders.BANK_NAME) placeholders.BANK_NAME = cData.bankDetails.bankName || '';
          if (!placeholders.BANK_ACCOUNT) placeholders.BANK_ACCOUNT = cData.bankDetails.accountNumber || '';
          if (!placeholders.IFSC_CODE) placeholders.IFSC_CODE = cData.bankDetails.ifscCode || '';
        }
      }
    } catch {
      // Fallback
    ...
    if (!placeholders.SIGNATORY_NAME) placeholders.SIGNATORY_NAME = defaultSignatoryName || 'Authorized Signatory';

    const htmlContent = compileInvoiceHTML({
      blocks: [],
      placeholders,
      brandLogoUrl,
      templateType,
