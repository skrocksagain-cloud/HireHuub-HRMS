...
export interface GenerateNativeDocumentPayload {
  brandId: string;
  documentType: string;
  entityId: string;
  requestId?: string;
...
export interface GenerateNativeDocumentResponse {
  success: boolean;
  requestId: string;
 ...
 */
function buildNativeOfferPlaceholders(payload: GenerateNativeDocumentPayload): Record<string, string> {
 ...
 */
export const generateNativeDocument = onCall<GenerateNativeDocumentPayload>(
  {
    memory: '1GiB',
    timeoutSeconds: 120,
    cors: true,
  },
  async (request): Promise<GenerateNativeDocumentResponse> => {
    ...
    try {
      ...
      if (!isAuthDisabled) {
        if (!request.auth) {
          throw new HttpsError('unauthenticated', 'Firebase authentication required to generate documents.');
        }
        ...
        if (employeeData) {
          ...
          if (accountStatus === 'Locked' || accountStatus === 'Inactive' || accountStatus === 'Terminated') {
            throw new HttpsError('permission-denied', `Account status '${accountStatus}' is not authorized to generate documents.`);
          }
          ...
          if (!isSuper) {
            ...
            if (!rolesQuery.empty) {
              ...
              if (!isAuthorized) {
                throw new HttpsError('permission-denied', `Role '${roleName}' is not authorized to generate offer documents.`);
              }
            }
          }
        }
      ...
      let result;
      const { NativeDocumentEngine } = await import('./nativeDocumentEngine');
      ...
      if (payload.documentType === 'RELIEVING_LETTER') {
        ...
        if (!personNameVal) {
          throw new HttpsError('invalid-argument', 'Relieving Letter generation failed: Employee name (PERSON_NAME) is required.');
        }
        if (!relievingRefVal) {
          throw new HttpsError('invalid-argument', 'Relieving Letter generation failed: Relieving reference number (RELIEVING_REF) is required.');
        }
        if (!lastWorkingDateVal) {
          throw new HttpsError('invalid-argument', 'Relieving Letter generation failed: Last working date (LAST_WORKING_DATE) is required.');
        }
        ...
        if (!personNameVal) {
          throw new HttpsError('invalid-argument', 'Increment Letter Generation Failed: Employee Name (PERSON_NAME) is required.');
        }
        ...
        const data = payload.data || {};
        const templateType = (data.templateType || 'All') as any;
        const placeholders: Record<string, string> = {
          LEGAL_NAME: String(data.legalName || data.companyName || ''),
          COMPANY_NAME: String(data.legalName || data.companyName || ''),
          BRAND_NAME: String(data.brandName || 'Hire Huub'),
        ...
        throw new HttpsError(
          'invalid-argument',
          `Document Generation Failed: Unsupported documentType '${payload.documentType}'. Supported document types: OFFER_LETTER, RELIEVING_LETTER, INCREMENT_LETTER, PAYSLIP, INVOICE.`
        );
      }
      ...
      const code = err instanceof HttpsError ? err.code : 'NATIVE_GENERATION_FAILED';
      return {
        success: false,
        requestId: reqId,
        documentId: null,
