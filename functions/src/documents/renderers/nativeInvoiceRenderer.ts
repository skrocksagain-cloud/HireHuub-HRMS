import type { RenderOfferOptions } from './nativeOfferRenderer';
...
export function validateHireHuubHSN(hsn: string): boolean {
  if (!hsn || !hsn.trim()) return true; // Optional allowed
 ...
 */
export function compileInvoiceHTML(options: RenderOfferOptions & { templateType?: HireHuubTemplateType }): string {
  ...
  let rawItems: Array<{ description: string; hsn: string; workDuration?: string; quantity?: number; unit?: string; unitPrice?: number; amount: number }> = [];
  try {
