export interface OfferBlockFormatting {
  fontSize?: number;
...
export interface OfferBlockHeaderConfig {
...
export interface OfferBlockFooterConfig {
...
export interface OfferBlock {
  id: string;
  type: 'header' | 'heading' | 'paragraph' | 'text' | 'table' | 'divider' | 'logo' | 'footer' | 'signature' | 'stamp' | 'page_break';
  title?: string;
  content?: string;
  ...
  signatoryId?: string;
  headerConfig?: OfferBlockHeaderConfig;
  footerConfig?: OfferBlockFooterConfig;
  formatting?: OfferBlockFormatting;
}
...
export interface RenderOfferOptions {
  ...
  specificSignatures?: Record<string, { signatureUrl: string; fullName: string; designation: string }>;
...
export function compileOfferLetterHTML(options: RenderOfferOptions): string {
  const {
    blocks,
    placeholders: p,
    brandLogoUrl = '',
  ...
  const renderPageFooterHTML = (pageIndex: number): string => {
    ...
    if (webText) rightBits.push(`<span>${webText}</span>`);
    if (pageNumStr) rightBits.push(`<span>${pageNumStr}</span>`);
  ...
  blocks.forEach((block) => {
    ...
    if (block.type === 'header' || (block.type === 'logo' && (block.headerConfig || block.title === 'Header & Legal Details'))) {
      ...
      const hCfg: OfferBlockHeaderConfig = block.headerConfig || {};
      ...
      if (showPhone && (p.BRAND_PHONE || p.PHONE)) contactBits.push(`Ph: ${escapeHTML(p.BRAND_PHONE || p.PHONE || '')}`);
      if (showEmail && (p.BRAND_EMAIL || p.EMAIL)) contactBits.push(`Email: ${escapeHTML(p.BRAND_EMAIL || p.EMAIL || '')}`);
      if (showWeb && (p.BRAND_WEBSITE || p.WEBSITE)) contactBits.push(`Web: ${escapeHTML(p.BRAND_WEBSITE || p.WEBSITE || '')}`);
      if (contactBits.length > 0) {
        legalLines.push(contactBits.join(' | '));
      }
      const taxBits: string[] = [];
      if (showCin && p.CIN) taxBits.push(`CIN: ${escapeHTML(p.CIN)}`);
      if (showPan && p.PAN) taxBits.push(`PAN: ${escapeHTML(p.PAN)}`);
      if (showGstin && p.GSTIN) taxBits.push(`GSTIN: ${escapeHTML(p.GSTIN)}`);
      if (taxBits.length > 0) {
