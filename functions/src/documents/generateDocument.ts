...
import { backendPlaceholderEngine, type PlaceholderContext } from '../engine/placeholderEngine';
...
function sanitizePayload<T extends Record<string, unknown>>(obj: T): T {
  ...
  for (const [key, value] of Object.entries(obj)) {
    if (value === undefined) {
      ...
      cleanObj[key] = sanitizePayload(value as Record<string, unknown>);
    } else {
      cleanObj[key] = value;
    }
  }
...
export const generateDocument = onCall<GenerateDocumentPayload>(
  {
    cors: true,
  },
  async (request) => {
    ...
    if (!docType) {
      throw new HttpsError(
        'invalid-argument',
        "Document Generation Error: 'documentType' is required."
      );
    }
    ...
    try {
      ...
      const snapshot = await adminDb.collection('document_templates').doc(slug).get();
      ...
      if (!templateData) {
        throw new HttpsError(
          'not-found',
          `No template found for '${docType}'. Please configure one in Administration → Document Templates via the Universal Template Builder.`
        );
      }
      ...
      if (state !== 'Published' && templateData.status === 'Inactive') {
        throw new HttpsError(
          'failed-precondition',
          `Template for '${docType}' is currently in state '${state}'. Only Published templates can be used for document generation.`
        );
      }

      // 2. Resolve Legal Company Entity & Brand Profiles
      const companyDoc = await adminDb.collection('admin_company').doc('hirehuub_company_settings').get();
      ...
      const foundBrand = ((companyData.brandProfilesList as BrandProfileItem[]) || []).find((b) => b.id === templateData?.brandProfileId);

      const brandProfile: BrandProfileItem = {
        id: foundBrand?.id || 'default',
        brandName: foundBrand?.brandName || (companyData.brandName as string) || (companyData.companyName as string) || 'HireHuub Enterprise ERP',
        logoUrl: foundBrand?.logoUrl || (companyData.logoUrl as string) || '',
        email: foundBrand?.email || (companyData.email as string) || 'hr@hirehuub.com',
        phone: foundBrand?.phone || (companyData.phone as string) || '+91 22 4900 1200',
        website: foundBrand?.website || (companyData.website as string) || 'https://hirehuub.com',
        address: foundBrand?.address || (companyData.address as string) || 'Corporate Financial Center, BKC, Mumbai, Maharashtra 400051',
      };

      const legalEntity = (companyData.legalEntity as { legalCompanyName?: string; cin?: string; pan?: string; gstin?: string; registeredOfficeAddress?: string }) || {
        legalCompanyName: (companyData.companyName as string) || 'HireHuub HR Solutions Private Limited',
        cin: (companyData.cin as string) || 'U74999MH2026PTC384920',
        pan: (companyData.pan as string) || 'AAACH9042K',
        gstin: (companyData.gstin as string) || '27AAACH9042K1Z5',
        registeredOfficeAddress: (companyData.address as string) || 'Corporate Financial Center, BKC, Mumbai, Maharashtra 400051',
      };

      const mergedContext: PlaceholderContext = {
        company: {
          ...companyData,
      ...
      const resolvedPlaceholders = backendPlaceholderEngine.resolvePlaceholders(mergedContext);
      ...
      await adminDb.collection('documents').doc(documentId).set(cleanData, { merge: true });
      ...
      throw new HttpsError(
        'internal',
        err instanceof Error ? err.message : 'An error occurred during Cloud Function document generation.'
      );
    }
  }
);
