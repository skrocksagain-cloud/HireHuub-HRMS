...
function isValidGoogleFileId(id: string): boolean {
  ...
  if (trimmed.startsWith('tmpl-') || trimmed.startsWith('doc-') || trimmed.startsWith('offer-') || trimmed.startsWith('tpl-')) return false;
 ...
 */
function extractGoogleFileId(val?: string | null): string {
 ...
 */
function extractTemplateIdFromRecord(data: Record<string, unknown>): string {
  if (!data) return '';
  const builderSchema = data.builderSchema as Record<string, unknown> | undefined;
...
export class TemplateResolver {
  ...
  static async resolveTemplate(
    ...
    const brandProfilesList = (companyData.brandProfilesList || []) as Array<{
      id: string;
      documentStorage?: Record<string, { folderId?: string; templateId?: string }>;
      documentTemplates?: Record<string, { templateId?: string }>;
    }>;
    ...
    switch (documentType) {
      case 'OFFER_LETTER': {
        ...
        if (!templateId) {
          const brandTemplatesSnap = await adminDb
        ...
        if (!templateId) {
          const defaultTemplatesSnap = await adminDb
            .collection('document_templates')
        ...
        if (!templateId) {
          const allTemplatesSnap = await adminDb.collection('document_templates').get();
      ...
      case 'PAYSLIP': {
        const rawBrandTmpl =
          brand?.documentTemplates?.[documentType]?.templateId ||
      ...
      case 'INVOICE': {
        ...
        if (!templateId) {
          const rawBrandInvoice =
            brand?.documentTemplates?.INVOICE?.templateId ||
