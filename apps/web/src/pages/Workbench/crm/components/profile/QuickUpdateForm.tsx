...
export default function QuickUpdateForm({
  ...
  openings,
  onSubmitUpdate,
}: Omit<QuickUpdateDrawerProps, 'isOpen' | 'onClose'>) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotes(''); // Clear notes for next interaction
      setInteractionId(crypto.randomUUID()); // Reset interaction ID for next update
    } catch (err: unknown) {
  ...
  return (
    <div className="flex flex-col h-full relative">
      {submitSuccess && (
        <div className="absolute top-0 left-0 right-0 z-10 p-3 bg-emerald-100 border border-emerald-300 rounded-xl text-emerald-800 text-xs font-bold text-center flex items-center justify-center gap-2 mb-4 animate-fade-out">
          <Check size={16} /> Update Saved Successfully
        </div>
      )}
      <form id="quick-update-form" onSubmit={handleSubmit} className="flex-1 space-y-4 pt-2">
          {formError && (
          ...
          <div className="pt-4 border-t border-slate-200 mt-4 flex items-center justify-end">
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold shadow-xs transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
