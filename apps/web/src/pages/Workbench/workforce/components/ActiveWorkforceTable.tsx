import { ExternalLink, History, Award } from 'lucide-react';
              ...
              {workforce.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/80 transition">
                  ...
                  <td className="py-3.5 px-4 text-slate-600 font-medium">
                    {item.workforceType === 'OTS' ? (
                      item.lastWorkingDate ? (
                        ...
                        <input
                          type="date"
                          onChange={(e) => onUpdateLwd?.(item.id, item.phone, item.candidateId, e.target.value)}
                          className="text-xs px-2 py-1 border border-slate-200 rounded-md text-slate-700 bg-slate-50 focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                      )
                    ) : (
                  ...
                  <td className="py-3.5 px-4 text-center font-bold text-slate-900">
                    {item.workforceType === 'OTS' ? (
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-[10px] font-medium text-slate-600">{item.tenureDisplay || `${item.tenureDays} Days`}</span>
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold border ${item.eligibility === 'Eligible' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                          {item.eligibility}
                        </span>
                      </div>
                    ) : (
                  ...
                  <td className="py-3.5 px-4 text-center">
                    {item.workforceType === 'Payroll' ? (
                      <span className="font-bold text-blue-700">{item.totalOrders || 0}</span>
                    ) : (
                      <span className="text-slate-400 text-[10px]">—</span>
                    )}
                  </td>
                  ...
                  {/* Rank */}
                  <td className="py-3.5 px-4 text-center">
                    {item.workforceType === 'Payroll' && item.rank !== undefined ? (
